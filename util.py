...
def check_link(content):
    surl = re.compile(r'[a-zA-z]+://pan.baidu.com/s/1([A-Za-z0-9_-]*)').findall(content)[0]
    pwd = re.compile(r'密码\W*\s*([A-Za-z0-9]{4})').findall(content)
    if len(pwd) > 0:
        pwd = pwd[0]
...
if __name__ == '__main__':
    ...
    # df['available'] = df.apply(lambda x: check_link(x['content']), axis=1)
    # print(df.head())
    # df.to_csv(csv)
